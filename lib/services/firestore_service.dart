...
import 'package:refsure/core/models/public_profile.dart';
...
class FirestoreService {
  ...
  CollectionReference get _publicProfiles => _db.collection('publicProfiles');
  ...
  Future<void> markOrgVerified(
          String uid, String orgEmail, String companyName) =>
      _users.doc(uid).update({
        'orgVerified': true,
        'orgEmail': orgEmail,
        'company': companyName,
  ...
  Stream<List<AppUser>> watchProviders() =>
      _publicProfiles.where('role', isEqualTo: 'provider').snapshots().map((s) {
        final list = s.docs
            .map(PublicProfile.fromFirestore)
            .map((profile) => profile.toAppUser())
            .toList();
  ...
  Stream<List<AppUser>> watchSeekers() =>
      _publicProfiles.where('role', isEqualTo: 'seeker').snapshots().map((s) {
        final list = s.docs
            .map(PublicProfile.fromFirestore)
            .map((profile) => profile.toAppUser())
            .toList();
  ...
  Stream<List<Job>> watchActiveJobs() =>
      _jobs.where('status', isEqualTo: 'active').limit(30).snapshots().map((s) {
  ...
  Stream<List<Job>> watchHotJobs() => _jobs
  ...
  Stream<List<Job>> watchProviderJobs(String providerId) => _jobs
  ...
  Future<String> postJob(Job job) async {
    final ref = job.id.isEmpty ? _jobs.doc() : _jobs.doc(job.id);
  ...
  Stream<List<Application>> watchSeekerApplications(String seekerId) =>
      _apps.where('seekerId', isEqualTo: seekerId).snapshots().map((s) {
  ...
  Stream<List<Application>> watchJobApplications(String jobId) =>
      _apps.where('jobId', isEqualTo: jobId).snapshots().map((s) {
        final list = s.docs.map(Application.fromFirestore).toList();
  ...
  Stream<List<Application>> watchProviderApplications(String providerId) =>
      _apps.where('providerId', isEqualTo: providerId).snapshots().map((s) {
        final list = s.docs.map(Application.fromFirestore).toList();
        list.sort((a, b) => b.matchScore.compareTo(a.matchScore));
        return list;
      });
  ...
  Future<void> updateApplicationStatus(String appId, AppStatus status,
      {String? note}) async {
  ...
  Stream<List<Message>> watchConversation(String uid1, String uid2) => _db
          .collection('messages')
          .where(Filter.or(
            Filter.and(Filter('fromId', isEqualTo: uid1),
                Filter('toId', isEqualTo: uid2)),
            Filter.and(Filter('fromId', isEqualTo: uid2),
                Filter('toId', isEqualTo: uid1)),
          ))
          .snapshots()
          .map((s) {
  ...
  Stream<Set<String>> watchBlockedUsers(String uid) => _users
      .doc(uid)
      .collection('blocks')
      .snapshots()
      .map((snapshot) => snapshot.docs.map((doc) => doc.id).toSet());

  Future<void> blockUser(String uid, String blockedUserId) =>
      _users.doc(uid).collection('blocks').doc(blockedUserId).set({
        'ownerId': uid,
        'blockedUserId': blockedUserId,
        'createdAt': FieldValue.serverTimestamp(),
      });

  Future<void> unblockUser(String uid, String blockedUserId) =>
      _users.doc(uid).collection('blocks').doc(blockedUserId).delete();

  // ─────────────────── GRATITUDES ─────────────────────────

  /// All gratitudes received by [referrerId], newest first.
  Stream<List<Gratitude>> watchGratitudesFor(String referrerId) => _gratitudes
  ...
  Stream<List<Gratitude>> watchAllGratitudes() => _gratitudes
  ...
  Stream<List<AppNotification>> watchNotifications(String userId) =>
      _notifs.where('userId', isEqualTo: userId).snapshots().map((s) {
    ...
    final sampleJobs = [
      {
        ...
        'preferredSkills': ['GCP', 'gRPC'],
        'tags': ['backend', 'infra'],
        'description':
            'Join Google Infrastructure to build distributed systems at planetary scale. '
      ...
      },
      {
        ...
        'preferredSkills': ['Enterprise Sales', 'Agile'],
        'tags': ['product', 'cloud'],
        'description':
            'Drive product strategy for Azure enterprise offerings in India. '
      ...
      },
      {
        ...
        'preferredSkills': ['Kafka', 'DynamoDB'],
        'tags': ['java', 'aws', 'backend'],
        'description':
            'Build the Prime membership platform serving 300M+ customers. '
