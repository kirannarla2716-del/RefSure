...
import '../core/policies/provider_job_policy.dart';
import '../core/policies/application_transition_policy.dart';
...
class _ApplicationsScreenState extends State<ApplicationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered =
        _tab == 'all' ? all : all.where((a) => a.statusKey == _tab).toList();
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Container(
            ...
            child: SingleChildScrollView(
                ...
                child: Row(
                  ...
                  final cnt = t.$3 == null
                      ? all.length
                      : all.where((a) => a.statusKey == t.$1).length;
                  final on = _tab == t.$1;
                  return Padding(
        ...
        Container(
            ...
            child: _ResponsiveFourItemLayout(children: [
              _Strip(
                  '${all.where((a) => a.status == AppStatus.strongMatch).length}',
                  'Strong Match',
                  AppColors.primary),
              _Strip(
                  '${all.where((a) => a.status == AppStatus.shortlisted).length}',
                  'Shortlisted',
                  AppColors.accent),
              _Strip(
                  '${all.where((a) => a.status == AppStatus.referred).length}',
                  'Referred',
                  AppColors.emerald),
              _Strip(
                  '${all.where((a) => a.status == AppStatus.notSelected || a.status == AppStatus.closed || a.status == AppStatus.hired || a.status == AppStatus.declined || a.status == AppStatus.withdrawn || a.status == AppStatus.expired).length}',
                  'Completed',
                  AppColors.gold),
            ])),
            ...
            child: filtered.isEmpty
                ? EmptyState(
                    ...
                    title: _tab == 'all'
                        ...
                        : 'None in this status',
                    subtitle: 'Browse jobs and request a referral',
                    action: _tab == 'all'
                        ? ElevatedButton(
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      return ApplicationCard(
                        app: app,
                        job: job,
                        onWithdraw: () => _withdraw(context, prov, app),
                      );
                    })),
      ]),
    );
  ...
  Future<void> _withdraw(
    ...
    final success = await provider.updateApplicationStatus(
      application.id,
      AppStatus.withdrawn,
      note: 'Withdrawn by the seeker.',
      expectedVersion: application.version,
    );
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(success
          ? 'Referral request withdrawn.'
          : provider.error ?? 'Could not withdraw the request.'),
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('Provider Jobs'),
          bottom: TabBar(
            ...
            tabs: const [
              ...
              Tab(text: 'Organization Jobs'),
            ],
          ),
        ),
        body: TabBarView(children: [
          const _ProviderPositionsView(),
          BlocProvider(
            create: (_) => getIt<CareersPortalCubit>(),
            child: CareersPortalScreen(
              initialCompany: user.currentCompany ?? user.company,
              embedded: true,
            ),
          ),
        ]),
...
class _ProviderPositionsView extends StatelessWidget {
  const _ProviderPositionsView();
  ...
  Widget build(BuildContext context) {
    ...
    final jobs = ProviderJobPolicy.ownedJobs(provider.allJobs, user.id);
    ...
    return ListView(
      ...
      children: [
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text('Keep positions current',
                      style: GoogleFonts.inter(
                          fontWeight: FontWeight.w700, color: Colors.white)),
                  Text(
                    'Open Organization Jobs to fetch the latest roles from ${user.currentCompany ?? user.company ?? 'your company'} and post with one click.',
                    style:
        ...
        Row(children: [
          Text('${jobs.length} posted position${jobs.length == 1 ? '' : 's'}',
              style:
          ...
          TextButton.icon(
            ...
            label: const Text('Post manually'),
          ),
        ]),
        if (jobs.isEmpty)
          EmptyState(
            ...
            subtitle:
                'Use Organization Jobs to fetch current openings and post them to RefSure.',
            action: FilledButton(
              onPressed: () => DefaultTabController.of(context).animateTo(1),
              child: const Text('Open Organization Jobs'),
            ),
          )
        else
          ...jobs.map((job) {
            final applications = provider.providerApplications
                .where((application) => application.jobId == job.id)
                .toList();
            return Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(children: [
                      ...
                      StatusPill(
                          status: job.status, label: job.status.toUpperCase()),
                    ]),
                    ...
                    Row(children: [
                      ...
                      Text('${applications.length} candidate profiles',
                          style: GoogleFonts.inter(
                              fontWeight: FontWeight.w600,
                      ...
                      TextButton(
                        onPressed: () =>
                            context.push('/provider/candidates/${job.id}'),
                        child: const Text('Review candidates'),
                      ),
                    ]),
                    if (applications.isNotEmpty) ...[
                      const Divider(),
                      ...applications.take(3).map((application) {
                        final seeker = provider.findUser(application.seekerId);
                        return Padding(
                          padding: const EdgeInsets.symmetric(vertical: 3),
                          child: Row(children: [
                            UserAvatar(
                                name: seeker?.name ?? 'Candidate', size: 26),
                            const SizedBox(width: 8),
                            Expanded(child: Text(seeker?.name ?? 'Candidate')),
                            MatchBandPill(
                                band: application.matchReport?.band ??
                                    MatchReport.bandFromScore(
                                        application.matchScore),
                                label: '${application.matchScore}% match'),
                          ]),
                        );
                      }),
                    ],
...
class ProviderPositionCandidatesScreen extends StatelessWidget {
  const ProviderPositionCandidatesScreen({super.key, required this.jobId});
  final String jobId;
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<AppProvider>();
    final user = provider.currentUser;
    final job = provider.findJob(jobId);
    if (user == null ||
        job == null ||
        !provider.isProvider ||
        !ProviderJobPolicy.canManage(job, user.id)) {
      return const Scaffold(
        body: EmptyState(
          icon: Icons.lock_outline,
          title: 'Candidate access unavailable',
          subtitle: 'You can review candidates only for your own positions.',
        ),
      );
    }
    final applications = provider.providerApplications
    ...
    return Scaffold(
      ...
      body: applications.isEmpty
          ? const EmptyState(
              ...
              subtitle: 'New referral requests for this position appear here.',
            )
          : ListView.separated(
              padding: const EdgeInsets.all(16),
              ...
              itemBuilder: (_, index) {
                ...
                return _CandidateCard(
                  app: application,
                  seeker: provider.findUser(application.seekerId),
                  job: job,
                );
              },
            ),
        ...
        actions: [
          ...
          Padding(
              ...
              child: GestureDetector(
                  ...
                  child: UserAvatar(
                      name: user.name, photoUrl: user.photoUrl, size: 34))),
        ],
        bottom: TabBar(
            controller: _tabs,
            indicatorColor: Colors.white,
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      SectionCard(
        ...
        _ResponsiveFourItemLayout(children: [
          StatBox(
              label: 'Total Job Applications',
              value: '$total',
              valueColor: AppColors.primary),
          StatBox(
              label: 'Strong', value: '$strong', valueColor: AppColors.emerald),
          StatBox(
              label: 'Shortlisted',
              value: '$shortl',
    ...
    return Column(children: [
      ...
      Container(
          ...
          child: SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: Row(children: [
                _FilterTab('All', 'all', _filter,
                    () => setState(() => _filter = 'all')),
                _FilterTab('80+ Match', 'strong', _filter,
                    () => setState(() => _filter = 'strong')),
                _FilterTab('Pending', 'pending', _filter,
                    () => setState(() => _filter = 'pending')),
                _FilterTab('Shortlisted', 'shortlisted', _filter,
                    () => setState(() => _filter = 'shortlisted')),
                _FilterTab('Referred', 'referred', _filter,
                    () => setState(() => _filter = 'referred')),
              ]))),
...
class _CandidateCard extends StatefulWidget {
  ...
  const _CandidateCard(
      {required this.app, this.seeker, this.job, this.rankPosition});
  @override
...
class _CandidateCardState extends State<_CandidateCard> {
    ...
    return SectionCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          // Rank badge
          if (widget.rankPosition != null)
            Container(
                width: 22,
                height: 22,
                decoration: BoxDecoration(
                    color: widget.rankPosition! <= 3
                        ? AppColors.goldLight
                        : AppColors.bg,
                    shape: BoxShape.circle,
                    border: Border.all(color: AppColors.border)),
                alignment: Alignment.center,
                child: Text('${widget.rankPosition}',
                    style: GoogleFonts.inter(
                        fontSize: 10,
                        fontWeight: FontWeight.w700,
                        color: widget.rankPosition! <= 3
          ...
          Expanded(
              child: Column(
                ...
                Row(children: [
                  Flexible(
                      child: Text(seeker?.name ?? 'Unknown Seeker',
                          style: GoogleFonts.inter(
                              fontSize: 15, fontWeight: FontWeight.w700),
                          maxLines: 1,
        ...
        if (_expanded && report != null) ...[
          ...
          if (report.matchedSkills.isNotEmpty) ...[
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(width: 4),
              Expanded(
                  child: Text(report.matchedSkills.join(", "),
                      style: GoogleFonts.inter(
                          fontSize: 11,
          ...
          if (report.missingSkills.isNotEmpty) ...[
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(width: 4),
              Expanded(
                  child: Text('Missing: ${report.missingSkills.join(", ")}',
                      style: GoogleFonts.inter(
                          fontSize: 11,
        ...
        Row(children: [
          ...
          Expanded(
            ...
            reverse: true,
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              if (ApplicationTransitionPolicy.allows(
                  app.status, AppStatus.declined)) ...[
                _ActionBtn('Decline', AppColors.redLight, AppColors.red,
                    () => _declineWithReason(context)),
                const SizedBox(width: 6),
              ],
              if (ApplicationTransitionPolicy.allows(
                  app.status, AppStatus.accepted)) ...[
                _ActionBtn('Accept', AppColors.emeraldLight, AppColors.emerald,
                    () => _updateStatus(context, AppStatus.accepted)),
                const SizedBox(width: 6),
              ],
              if (!ApplicationTransitionPolicy.allows(
                      app.status, AppStatus.declined) &&
                  ApplicationTransitionPolicy.allows(
                      app.status, AppStatus.notSelected)) ...[
                _ActionBtn('Not selected', AppColors.redLight, AppColors.red,
              ...
              if (ApplicationTransitionPolicy.allows(
                  app.status, AppStatus.shortlisted)) ...[
                _ActionBtn(
                    'Shortlist',
  ...
  Future<void> _updateStatus(BuildContext context, AppStatus status) async {
    final provider = context.read<AppProvider>();
    final success = await provider.updateApplicationStatus(
      widget.app.id,
      status,
      expectedVersion: widget.app.version,
    );
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(success
          ? 'Referral request updated.'
          : provider.error ?? 'Could not update the referral request.'),
      backgroundColor: success ? AppColors.emerald : AppColors.red,
                      ...
                      children: [
                        ...
                        TextField(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'Internal referral reference',
                                hintText:
                                    'Requisition, ATS, or submission ID')),
                        const SizedBox(height: 12),
                        TextField(
                            controller: noteCtrl,
                            maxLines: 3,
                            decoration: const InputDecoration(
                                ...
                                hintText:
                                    'e.g. "Submitted your profile to the hiring manager..."')),
                        if (error != null) ...[
                          const SizedBox(height: 10),
                          Text(error!,
                              style: GoogleFonts.inter(
                                      ...
                                      : () async {
                                          ...
                                          if (receipt.isEmpty) {
                                            setModalState(() {
                                              submitting = false;
                                              error =
                                                  'Enter the internal referral reference.';
                                            });
                                            return;
                                          }
  ...
  void _declineWithReason(BuildContext context) {
    final reasonCtrl = TextEditingController();
    showDialog<void>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Decline referral request'),
        content: TextField(
          controller: reasonCtrl,
          maxLines: 3,
          decoration: const InputDecoration(
            labelText: 'Reason',
            hintText: 'Give the seeker a useful, professional reason',
          ),
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            onPressed: () async {
              ...
              final success =
                  await context.read<AppProvider>().updateApplicationStatus(
                        widget.app.id,
                        AppStatus.declined,
                        note: reason,
                        expectedVersion: widget.app.version,
      ...
      Expanded(
          child: jobs.isEmpty
              ? EmptyState(
                  ...
                  subtitle:
                      'Post jobs from your company careers portal or create manually',
  ...
  Widget build(BuildContext context) => SectionCard(
      ...
      child: Row(children: [
        Expanded(
          ...
          Text(
              '${job.location} · ${job.workMode} · ${job.minExp}–${job.maxExp} yrs',
              style:
...
class _ManualPostFormState extends State<_ManualPostForm> {
      ...
      ListView(padding: const EdgeInsets.all(16), children: [
        ...
        SectionCard(
          ...
          Text('Experience: ${_minExp.round()}–${_maxExp.round()} years',
              style:
                  GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600)),
          RangeSlider(
              values: RangeValues(_minExp, _maxExp),
              min: 0,
              max: 25,
              divisions: 25,
          ...
          Text('Salary: ${_salMin.round()}–${_salMax.round()} LPA',
              style:
                  GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600)),
          RangeSlider(
              values: RangeValues(_salMin, _salMax),
              min: 0,
              max: 200,
              divisions: 40,
              activeColor: AppColors.primary,
          ...
          Wrap(
              ...
              children: _commonSkills.map((s) {
                ...
                return FilterChip(
                    ...
                    selected: on,
                    onSelected: (_) =>
                        setState(() => on ? _skills.remove(s) : _skills.add(s)),
        ...
        SectionCard(
          ...
          Wrap(
              spacing: 6,
              runSpacing: 6,
              children: _commonTags.map((t) {
                final on = _tags.contains(t);
                return FilterChip(
                    label: Text('#$t'),
                    selected: on,
                    onSelected: (_) =>
                        setState(() => on ? _tags.remove(t) : _tags.add(t)),
        ...
        SectionCard(
          ...
          TextField(
              ...
              decoration: const InputDecoration(
                  hintText:
                      'Describe the role, responsibilities, team, and culture...',
        ...
        SectionCard(
          ...
          TextField(
              ...
              decoration: const InputDecoration(
                  hintText:
                      'e.g. Hiring manager prefers AWS experience over GCP...',
                  border: InputBorder.none)),
        ])),
        const SizedBox(height: 12),
        SectionCard(
          ...
          Row(children: [
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  Text('Mark as Hot / Urgent',
                      style: GoogleFonts.inter(fontSize: 14)),
                  Text('Shown with a Hot badge',
                      style: GoogleFonts.inter(
                          fontSize: 11, color: AppColors.textHint)),
                ])),
          ...
          const SizedBox(height: 12),
          _f('Careers Portal URL (optional)', _extUrl,
  ...
  Widget _f(String label, TextEditingController ctrl, String hint) => TextField(
      controller: ctrl,
  ...
  Future<void> _post() async {
    if (_title.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(
              'Fill in title, location, description, and at least one skill')));
      return;
    }
  ...
  Widget build(BuildContext context) =>
      ListView(padding: const EdgeInsets.all(16), children: [
        Container(
            ...
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Import from Company Careers',
                  style: GoogleFonts.inter(
                      fontSize: 15,
                      fontWeight: FontWeight.w700,
                      color: AppColors.primary)),
              const SizedBox(height: 6),
              Text(
                  'Enter your company careers page URL. We\'ll extract open positions '
        ...
        if (_fetchedJobs.isNotEmpty) ...[
          Row(children: [
            Text('Found ${_fetchedJobs.length} positions',
                style: GoogleFonts.inter(
                    fontSize: 14, fontWeight: FontWeight.w700)),
            const Spacer(),
            TextButton(
                onPressed: () => setState(() {
                      _selected
                          .addAll(List.generate(_fetchedJobs.length, (i) => i));
                    }),
                    ...
                    child: Row(children: [
                      ...
                      Expanded(
                          child: Column(
                            ...
                            Text(
                                '${e.value['location']} · ${e.value['department']}',
                                style: GoogleFonts.inter(
  ...
  Future<void> _publishSelected() async {
    ...
    for (final i in _selected) {
      ...
      await prov.postJob({
        ...
        'description':
            'Imported from ${_url.text.trim()}. Please update with full details.',
        'skills': <String>[],
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final user = prov.currentUser;
    if (user == null) {
      if (!prov.authReady || prov.loading) return const LoadingSpinner();
      ...
      return Scaffold(
        ...
        body: Center(
          ...
          Text('Your account exists but profile is missing.',
              style:
                  GoogleFonts.inter(fontSize: 13, color: AppColors.textSecond)),
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        if (prov.isAdmin || DemoModePolicy.isEnabled) ...[
          SectionCard(
            ...
            child: const ListTile(
              ...
              subtitle:
                  Text('Manage people, access, exceptions, and release gates'),
        ...
        if (isReferrer) ...[
          ...
          SectionCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Referral availability',
                    style: GoogleFonts.inter(
                        fontSize: 15, fontWeight: FontWeight.w700)),
                const SizedBox(height: 4),
                Text(
                  'Control whether seekers can send new requests.',
                  style: GoogleFonts.inter(
                      fontSize: 12, color: AppColors.textSecond),
                ),
                SwitchListTile.adaptive(
                  contentPadding: EdgeInsets.zero,
                  title: Text(user.availableForReferrals
                      ? 'Accepting referrals'
                      : 'Requests paused'),
                  subtitle: Text(
                    '${user.activeReferralRequests} active of '
                    '${user.weeklyReferralCapacity} weekly slots',
                  ),
                  value: user.availableForReferrals,
                  onChanged: (value) =>
                      context.read<AppProvider>().setReferralAvailability(
                            available: value,
                            weeklyCapacity: user.weeklyReferralCapacity,
                          ),
                ),
                Wrap(
                  spacing: 8,
                  children: [3, 5, 10, 15].map((capacity) {
                    return ChoiceChip(
                      label: Text('$capacity / week'),
                      selected: user.weeklyReferralCapacity == capacity,
                      onSelected: (_) =>
                          context.read<AppProvider>().setReferralAvailability(
                                available: user.availableForReferrals,
        ...
        SectionCard(
          ...
          _InfoLine(
              Icons.business_outlined, user.company ?? 'Add organisation'),
          if (user.title.isNotEmpty) _InfoLine(Icons.work_outline, user.title),
          if (user.location.isNotEmpty)
            _InfoLine(Icons.location_on_outlined, user.location),
          if (user.experience > 0)
            _InfoLine(
                Icons.timer_outlined, '${user.experience} years experience'),
          if (user.noticePeriod != null)
            _InfoLine(Icons.schedule_outlined, user.noticePeriod!),
          if (user.expectedSalary != null)
            _InfoLine(
                Icons.currency_rupee, '${user.expectedSalary} LPA expected'),
        ...
        if (user.skills.isNotEmpty) ...[
          ...
          SectionCard(
              child: Column(
                  ...
                  children: [
                SectionHeader(
                    title: 'Skills (${user.skills.length})',
                    action: IconButton(
        ...
        SectionCard(
          ...
          const SizedBox(height: 4),
          Text(
            'Requests are securely queued for RefSure operations and tracked to completion.',
          ...
          _SupportRow(
            ...
            value: 'Receive an export of your RefSure account data',
            onTap: () => _requestPrivacyAction(context, 'data_export'),
          ),
          const Divider(height: 20),
          _SupportRow(
            icon: Icons.delete_forever_outlined,
            label: 'Request account deletion',
            value: 'Start a verified account and data deletion request',
            onTap: () => _requestPrivacyAction(context, 'account_deletion'),
          ),
        ])),
        ...
        SectionCard(
          ...
          _SupportRow(
              ...
              onTap: () {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('Email us at ${AppConstants.supportEmail}')));
              }),
          const Divider(height: 20),
          _SupportRow(
              ...
              onTap: () {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('Call us at ${AppConstants.supportPhone}')));
              }),
          const Divider(height: 20),
          _SupportRow(
              ...
              onTap: () async {
                ...
                if (context.mounted) {
                  context.go('/');
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content: Text('Quick Tour is back on your Home screen')));
                }
              }),
        ])),
  ...
  Future<void> _requestPrivacyAction(
    BuildContext context,
    String type,
  ) async {
    final deleting = type == 'account_deletion';
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title:
            Text(deleting ? 'Request account deletion?' : 'Request your data?'),
        content: Text(deleting
            ? 'RefSure support will verify this request before deleting your account and eligible data.'
            : 'RefSure support will prepare an export and contact you at your account email.'),
        actions: [
          TextButton(
    ...
    try {
      if (!DemoModePolicy.isEnabled) {
        await TrustedAccountService().requestPrivacyAction(type);
      }
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(deleting
  ...
  void _confirmRoleSwitch(BuildContext context, UserRole target) {
    ...
    final label = RoleLabels.name(target);
    showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text('Switch to $label?'),
        content: Text(target == UserRole.provider
          ...
          FilledButton(
              onPressed: () async {
                ...
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content:
                        Text(prov.error ?? 'Could not switch account mode.'),
...
class _ResponsiveFourItemLayout extends StatelessWidget {
  final List<Widget> children;
  const _ResponsiveFourItemLayout({required this.children});
...
class _DeveloperSectionState extends State<_DeveloperSection> {
  ...
  Future<void> _seed() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _msg =
          'Seed data written — reload the app to see all screens populated.');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Account mode',
              style:
                  GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w700)),
          const SizedBox(height: 4),
          Text('Switch between Referral-SEEKER and Referral-PROVIDER.',
              style:
                  GoogleFonts.inter(fontSize: 12, color: AppColors.textSecond)),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(
                child: _RoleToggle(
                    ...
                    label: RoleLabels.name(UserRole.seeker),
            ...
            Expanded(
                child: _RoleToggle(
                    ...
                    label: RoleLabels.name(UserRole.provider),
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Text(label,
                style: GoogleFonts.inter(
                    ...
                    fontWeight: FontWeight.w600,
                    color:
                        selected ? AppColors.primary : AppColors.textSecond)),
            if (selected) ...[
              const SizedBox(width: 6),
  ...
  Widget build(BuildContext context) {
    ...
    if (prov.isProvider) {
      ...
      contacts = prov.providerApplications
          ...
          .map((a) {
            try {
              return prov.findUser(a.seekerId);
            } catch (_) {
              return null;
            }
          })
      ...
      contacts = prov.myApplications
          ...
          .map((a) {
            try {
              return prov.findUser(a.providerId);
            } catch (_) {
              return null;
            }
          })
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final uid = prov.currentUser?.id ?? '';
    final blocked = prov.isUserBlocked(widget.otherId);
    ...
    return Scaffold(
      appBar: AppBar(
          title: other != null
              ? Row(children: [
                  UserAvatar(
                      name: other.name, photoUrl: other.photoUrl, size: 32),
                  const SizedBox(width: 10),
                  Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
          ...
          actions: [
            PopupMenuButton<String>(
              ...
              onSelected: (value) {
                ...
                if (value == 'block') _toggleBlock(blocked);
              },
              itemBuilder: (_) => [
                const PopupMenuItem(
                  value: 'report',
      ...
      body: Column(children: [
        ...
        if (blocked)
          Container(
            ...
            child: Row(children: [
              const Icon(Icons.block, size: 18, color: AppColors.red),
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Message could not be sent. Try again.')),
      );
      return;
    }
  ...
  Future<void> _toggleBlock(bool currentlyBlocked) async {
    if (currentlyBlocked) {
      await context.read<AppProvider>().unblockUser(widget.otherId);
      return;
    }
    final confirmed = await showDialog<bool>(
      context: context,
    ...
    if (confirmed == true && mounted) {
      await context.read<AppProvider>().blockUser(widget.otherId);
    }
  }
  ...
  Future<void> _showReportDialog() async {
    ...
    final error = await context.read<AppProvider>().reportUser(
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(error ?? 'Report submitted for review.'),
    ));
  }
}
