...
class ProviderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final hasCapacity =
        provider.activeReferralRequests < provider.weeklyReferralCapacity;
    final isAvailable = provider.availableForReferrals && hasCapacity;
    return SectionCard(
      onTap: () => context.push(RouteNames.providerDetailPath(provider.id)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          UserAvatar(
              name: provider.name, photoUrl: provider.photoUrl, size: 48),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            _AvailabilityPill(
              label: isAvailable
                  ? '${provider.weeklyReferralCapacity - provider.activeReferralRequests} slots'
                  : provider.availableForReferrals
                      ? 'At capacity'
                      : 'Paused',
        ...
        Wrap(spacing: 6, runSpacing: 4, children: [
          ...
          if (provider.skills.length > 3)
            Text('+${provider.skills.length - 3}',
                style:
...
class _AvailabilityPill extends StatelessWidget {
  ...
  const _AvailabilityPill({required this.label, required this.available});

  @override
  Widget build(BuildContext context) => Container(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
        decoration: BoxDecoration(
          color: available ? AppColors.emeraldLight : AppColors.bg,
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(
            ...
            size: 12,
            color: available ? AppColors.emerald : AppColors.textHint,
          ),
